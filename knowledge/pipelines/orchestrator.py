"""Canonical knowledge pipeline orchestration for compatibility facades."""
...
def normalize_markdown_text(content: str) -> str:
    """Normalize markdown text using the same rules as the markdown adapter."""
...
def _build_default_registry() -> OntologyRegistry:
...
class KnowledgePipelineArtifacts:
    """Artifacts produced by the compatibility knowledge pipeline."""
...
def run_knowledge_pipeline(
    content: str,
    *,
    task: str = "Knowledge pipeline compatibility qualification",
    query_text: str = "chamber pressure LOX",
    request_id: str = "compat-pipeline",
    source_id: str = "SRC-GOLDEN",
    artifact_id: str = "ART-GOLDEN",
    extra_entities: tuple[CandidateEntityExtraction, ...] = (),
    ontology_registry: OntologyRegistry | None = None,
) -> KnowledgePipelineArtifacts:
    """Execute the canonical W1→W11 path for compatibility consumers."""
    ...
    cursor_context = CursorContextBuilder().build(
        project_id="COSMOS",
        engineering_task_id="BLOCK-013-COMPAT",
        package=package,
